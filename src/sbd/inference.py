...
def get_sentences(text):
    """
    Your 'segment_text' function, renamed to be standard for Phase 2.
    """
    if not crf_model: return [text] # Fallback if model fails
    ...
    if current_idx < len(tokens_with_spans):
        sentences.append(text[tokens_with_spans[current_idx][1]:].strip())
